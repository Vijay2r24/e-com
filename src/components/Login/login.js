...
// import {COUNTRIES_API,STATES_API,CITIES_API,Login } from "../../Constants/apiRoutes"
...
const fetchApiData = async () => {
  ...
  if (!storedCitiesData || !storedStatesData || !storedCountriesData || !storedOrderStatusData) {
    try {
      ...
      // Fetch API data in parallel using Promise.all
      const [resCities, resStates, resCountries, resOrderStatus] = await Promise.all([
        storedCitiesData ? JSON.parse(storedCitiesData) : fetch(CITIES_API, fetchOptions).then((res) => res.json()),
        storedStatesData ? JSON.parse(storedStatesData) : fetch(STATES_API, fetchOptions).then((res) => res.json()),
        storedCountriesData ? JSON.parse(storedCountriesData) : fetch(COUNTRIES_API, fetchOptions).then((res) => res.json()),
