...
import {COUNTRIES_API,STATES_API,CITIES_API,getAllOrderStatus,Login} from "../../Constants/apiRoutes"
...
const fetchApiData = async () => {
  ...
  if (!storedCitiesData || !storedStatesData || !storedCountriesData || !storedOrderStatusData) {
    try {
      const fetchOptions = {
        headers: {
      ...
      const [cities, states, countries, orderStatus] = await Promise.all([
        storedCitiesData ? JSON.parse(storedCitiesData) : fetch(CITIES_API, fetchOptions).then(res => res.ok ? res.json() : null),
        storedStatesData ? JSON.parse(storedStatesData) : fetch(STATES_API, fetchOptions).then(res => res.ok ? res.json() : null),
        storedCountriesData ? JSON.parse(storedCountriesData) : fetch(COUNTRIES_API, fetchOptions).then(res => res.ok ? res.json() : null),
        storedOrderStatusData ? JSON.parse(storedOrderStatusData) : fetch(getAllOrderStatus, fetchOptions)
          .then(res => res.ok ? res.json() : null)
          .then(data => data?.data || null), // Ensure correct data structure
      ]);
      ...
      if (orderStatus) localStorage.setItem("orderStatusData", JSON.stringify(orderStatus));
